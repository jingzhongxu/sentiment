...
2.增加了精准匹配的相关函数
3.在计算准确率的过程中，发现测试样本的个数应该是一样的，所有又设置了测试样本的个数上限是500,即都比500小
4.TestBaselineTrainResult中我计算召回率的时候，是所有的测试样本(没有因为平衡的个数限制)都是直接初始化数据库大小的，不适合现在的这个类
所以results的声明定义初始化需要更改，之前是在processResult中定义及初始化工作，同时借助initSingleResult函数，现在在当前类中的
构造函数完成定义及初始化工作
5.同时之前TestBaselineTrainResult类中，我在统计results的时候，是用的set方法(因为我不用管这个list的大小，它已经初始化好了)，当前类中我要改成
像accruacys一样的更新方法。
*/
public class TestBaselineTrainResultSecond extends TestBaselineTrainResult
{
	public ArrayList<ArrayList<Boolean>> accuracys;

	public TestBaselineTrainResultSecond(int parameterNums,String paraFile) throws Exception
	{
	...
	{
		TestBaselineTrainResultSecond obj = new TestBaselineTrainResultSecond(50829,"./result/trainResult_baseline.result");
