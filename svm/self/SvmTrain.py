...
class SvmTrain:
	...
	def __init__(self,corpusRoot=None,sentimentwordRoot=None,pickleRoot=None):
		if pickleRoot != None:
			with open(pickleRoot,"rb") as fr:
				self.trainPoints = pickle.load(fr)
		elif sentimentwordRoot==None or corpusRoot==None:
			print("init wrong!")
		else:
			self.trainPoints = ProcessCorpus.ProcessCorpus(corpusRoot,sentimentwordRoot)
			self.trainPoints.getPosNegAttributes()
	...
	def __computeLAndH(self,C,index_1,index_2):
	...
	def __updateThreshold_b(self,index_1,index2,a1_old,a1_new,a2_old,a2_new_cliped,L,H):
		b_old = self.b
		E1 = self.E[index_1]
		E2 = self.E[index_2]
		...
		if a1_new < H and a1_new >L : self.b = b1
		if a2_new_cliped<H and a2_new_cliped >L: self.b = b2
		if a1_new < H and a1_new >L and  a2_new_cliped<H and a2_new_cliped >L : self.b = b1
		else : self.b = (b1+b2)/2 
		...
		for index,E_index in enumerate(self.E):
			if index not in nonBoundSet:
				...
				if index==index_1 or index==index_2:
					E_index_new = 0 + y1*(a1_new-a1_old)*self.kernelMatrix(index,index_1) + y2 *(a2_new_cliped - a2_old)*self.kernelMatrix(index,index_2) + b_old - b_new
					slef.E[index] = E_index_new
				else:
	...
	def takestep(self,index_1,index_2,nonBoundSet):
		...
		b_old = self.b
		s = y1*y2
		L,H = self.__computeLAndH(C,index_1,index_2)
		if L==H: return 0
		k11 = self.kernelMatrix[index_1,index_1]
		k12 = self.kernelMatrix[index_1,index_2]
		k22 = self.kernelMatrix[index_2,index_2]
		...
		self.__updateThreshold_b(index_1,index2,a1_old,a1_new,a2_old,a2_new_cliped,L,H)
		b_new = self.b
...
def test():
	obj = SvmTrain("./corpus/test","./corpus/fre4train.txt")
	print(len(obj.trainPoints.sentimentword))
	print(len(obj.trainPoints.negSamplePointList))
